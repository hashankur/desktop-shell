...
Scope {
    ...
    NotificationServer {
        ...
        onNotification: function(notification) {
            ...
            while (historyModel.count > root.maxHistory) historyModel.remove(historyModel.count-1)
            ...
            if (timeoutSec > 0) {
                var expireTimer = Qt.createQmlObject(
                    "import QtQuick; Timer { interval: " + (timeoutSec * 1000) + "; running: true; repeat: false }",
                    root
                )
    ...
    function clearHistory() {
        historyModel.clear()
        if (root.persistenceEnabled) saveHistory()
    }

    function saveHistory() {
        try {
            var arr = []
            for (var i=0;i<historyModel.count;i++) arr.push(historyModel.get(i))
            historyFile.setText(JSON.stringify(arr))
        } catch (e) { console.error("Failed to save notifications to history:", e) }
    }
        ...
        try {
            if (historyFile.loaded) {
                ...
                if (content && content.length > 0) {
                    ...
                    for (var i=0;i<arr.length;i++) historyModel.append(arr[i])
                }
            }
        } catch (e) { console.warn("No persisted notifications or failed to load:", e) }
    }
    ...
    function openHistory() {
        try {
            ...
            if (comp.status === Component.Ready) {
                ...
                if (!w) console.error("Failed to create NotificationHistory window")
            } else {
                console.warn("NotificationHistory component not ready:", comp.status)
            }
        } catch (e) { console.warn("openHistory failed:", e) }
    }

    Component.onCompleted: {
        // Load persisted history if enabled
        if (root.persistenceEnabled) loadHistory()
